...
启动界面不是固定时长的宣传图，而是主窗口尚未可交互时的真实状态反馈。它使用 Catnip Forge 品牌素材建立第一印象，并在工作台准备完成后自动让位给主窗口。
...
1. Electron ready 后先创建 Splash，初始状态为 8%。
2. 创建隐藏的主窗口时更新为 22%“正在准备工作区”。
3. Gateway 和 BrowserView 开始工作后更新为 48%“正在连接开发环境”。
4. Renderer 完成加载后更新为 84%“正在载入工作台”。
5. 主窗口触发 `ready-to-show` 后更新为 100%“准备就绪”，320ms 后关闭 Splash 并显示、聚焦主窗口。

不得把进度改为脱离加载节点的循环假进度。主页面加载失败时应关闭 Splash、显示主窗口错误页并记录日志，避免用户被永久困在启动界面。
...
`verify:splash-ui` 需要先以 `VIBEIDE_SPLASH_HOLD=1` 启动桌面开发版。脚本通过 CDP 验证窗口尺寸、三张素材、文案、进度更新与溢出状态，并把截图写入临时目录；截图不得提交 Git。

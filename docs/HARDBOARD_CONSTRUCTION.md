...
把奥德赛0.4.0-7171 重构为硬件 vibecoding 专用 IDE：
...
Windows 打包配置包含 `runtime/hardboard`，但排除 ESP-IDF 自带 `examples/**`，避免 NSIS/7zip 处理 Unix 脚本路径时报错。奥德赛0.4.0-7171 自己的 `runtime/hardboard/example/**` 保留。

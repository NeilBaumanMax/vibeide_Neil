...
2026-07-21 最终成品实测：目录共 `4,459,269,207` 字节；Node `v22.14.0`、Python 3.12/pyserial `3.5`、Claude Code `2.1.167` 和 Runtime health 均通过。成品实际启动后，首次配置窗口、Playwright 资源和 Skills 按钮均通过 CDP 校验，占位 Key 被拒绝，测试结束后 `resources/apikey.txt` 仍不存在。

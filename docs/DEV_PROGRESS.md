...
- [x] 猫薄荷产品知识从 TypeScript 硬编码迁移到随包 `CATNIP_FORGE_USER_GUIDE.md`；每次提问重新读取，支持免重启维护、60,000 字符上限和缺失/空文件安全降级
- [x] 新增 `verify:software-assistant-guide`，验证手册 A→B 动态更新不缓存，并覆盖空文件与缺失文件降级；发布校验确认手册位于可编辑的 `resources` 根目录

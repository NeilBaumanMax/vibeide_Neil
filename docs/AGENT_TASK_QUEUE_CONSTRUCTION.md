...
- turn 完成、页面验收和恢复任务等异步回调必须绑定原 `taskId`；停止或切换任务后到达的旧回调只能忽略，不能完成、失败或重新启动新任务。
...
7. 停止或任务切换后，旧 turn/页面验收回调不得修改当前任务状态，也不得复活已停止任务。
...
`verify:task-queue` 还会模拟 turn 完成回调已经开始、任务随即被取消的时序，确认旧回调恢复后不会把已取消任务标记完成；同时验证停止会清空追加要求和独立队列。
